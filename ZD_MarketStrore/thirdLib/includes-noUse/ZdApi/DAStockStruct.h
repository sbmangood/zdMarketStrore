...
namespace Directaccess {
	...
	struct CStockRspOrderField
	{
		...
		// 有效日期:
		//		STOCK_TDY_TIF=当日有效, STOCK_GTC_TIF=永久有效（GTC），STOCK_OPG_TIF=OPG,STOCK_IOC_TIF4=IOC
		//		STOCK_FOK_TIF=FOK，STOCK_GTD_TIF=GTD，STOCK_ATC_TIF=ATC，STOCK_FAK_TIF=FAK
		// Order time in force:
		//		STOCK_TDY_TIF=the day only,STOCK_GTC_TIF=GTC,STOCK_OPG_TIF=OPG,STOCK_IOC_TIF=IOC
		//		STOCK_FOK_TIF=FOK,STOCK_GTD_TIF=GTD,STOCK_ATC_TIF=ATC,STOCK_FAK_TIF=FAK		
	...
	struct CStockRspCapitalField
	{
		...
		TDAMoneyType		MortgageInMoney;					// 按揭价值 20150610 added for 港股
																// Mortgage in money(add for HK stock)

		TDAMoneyType		MarginLimit;						// 孖展上限额度 20150727 added for 港股
																// Margin limit(add for Hk stock)

		TDAMoneyType		BorrowInMoney;						// 借货价值 20150727 added for 港股
																// Borrow in money

		TDAMoneyType		T1DeliveryMoney;					// T1日交割的资金，正入负出
																// T1 day delivery of funds, positive in negative out

		TDAMoneyType		T2DeliveryMoney;					// T2 日交割的资金，正入负出
																// T2 day delivery of funds, positive in negative out

		TDAMoneyType		T3DeliveryMoney;					// T3 日交割的资金，正入负出
																// T3 day delivery of funds, positive in negative out

		TDAMoneyType		TNDeliveryMoney;					// TN 日交割的资金，正入负出
		...
		TDAMoneyType		CrossCurrencyMaxMoneyAmt;			// 跨市场资金限额
																// Cross-market capital limits

		TDAMoneyType		SellShortFrozenMoney;				// 卖空冻结资金
																// Short sale frozen funds

		TDAMoneyType		SellShortInterest;					// 卖空利息
																// Short sale interest

		TDAMoneyType		ShortPosAddtionalMargin;			// 需补按金
	...
	struct CStockRspPositionField
	{
		...
		TDAStringType		ProductCode;						// 证券代码
																// Product code
		// 持仓方向:
		...
		TDADoubleType		PosCostPrice;						// 持仓成本价
																// Position at cost

		TDAIntType			CanSellShares;						// 可卖股数
																// Can sell shares

		TDAIntType			TodayBuyShares;						// 今买数量
																// Today buy shares

		TDAIntType			FrosenShares;						// 冻结数量
																// Frosen shares
		...
		TDAIntType			TotalBuyShares;						// 持有期内买入总股数
																// Total number of shares bought during the holding period

		TDAIntType			TotalSellShares;					// 持有期内卖出总股数
																// Total number of shares sold during the holding period

		TDAStringType		FirstPosDate;						// 首次开仓日期(yyyy-MM-dd)
																// Date of first opening(yyyy-MM-dd)

		TDADoubleType		ClosePosPL;							// 平仓盈利
																// Closing position profit

		TDAIntType			T1DeliveryShares;					// T1日交割股数，正入负出
																// T1 day delivery of shares, positive in negative out

		TDAIntType			T2DeliveryShares;					// T2日交割股数，正入负出
																// T2 day delivery of shares, positive in negative out

		TDAIntType			T3DeliveryShares;					// T3日交割股数，正入负出
																// T3 day delivery of shares, positive in negative out

		TDAIntType			NotDeliveryShares;					// 未交割股数=T1DeliveryShares+T2DeliveryShares+T3DeliveryShares
																// Number of outstanding shares=T1DeliveryShares+T2DeliveryShares+T3DeliveryShares

		TDAIntType			DeliveredShares;					// 已交割股数
																// Number of shares delivered

		TDAIntType			SellShortShares;					// 卖空股数
																// Short the number of shares

		TDADoubleType		SellShortMoney;						// 卖空冻结资金
																// Short sale frozen funds

		TDADoubleType		SSPosAvgCostPrice;					// 卖空仓位的开仓均价
